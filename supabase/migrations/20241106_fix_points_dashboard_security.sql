-- CRITICAL SECURITY FIX: Points Dashboard RPC
-- 
-- VULNERABILITY: Previous version accepted p_user_id parameter from client,
-- allowing any user to view any other user's points data by manipulating the parameter.
--
-- FIX: Remove parameter and use auth.uid() to get authenticated user's ID directly from JWT.
-- This ensures users can ONLY access their own points data.
...
COMMENT ON FUNCTION get_points_dashboard() IS 
'Secure points dashboard data fetcher. Uses auth.uid() from JWT to ensure users can only access their own points data. 
